...
if ($result->num_rows > 0) {
    ...
    if ($data['est_conducteur']) {
        $update_sql = "UPDATE covoiturage SET statut = 'terminé' WHERE covoiturage_id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("i", $trip_id);
        $update_stmt->execute();
        
        $_SESSION['success_message'] = "🏁 Trajet terminé! Les passagers peuvent maintenant laisser un avis.";
    } else {
        $_SESSION['error_message'] = "Seul le conducteur peut terminer le trajet";
    }
} else {
